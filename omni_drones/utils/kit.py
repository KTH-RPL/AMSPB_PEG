...
def load_template_scene():
    prim_utils.create_prim(
        prim_path="/World/BasicScene",
        usd_path="./omni_drones/scenes/Templates/Basic/clean_cloudy_sky_and_floor.usd",
...
# )
    ...
    # Then rotate just the floor geometry afterwards
    # doom_light_prim_path = "/World/BasicScene/sky/DomeLight" 
    # prim_utils.set_prim_property(doom_light_prim_path, "Rotation", (360, 0, 0))

    prim_utils.set_prim_property("/World/BasicScene/sky/DomeLight", "xformOp:rotateZYX", (0, 0, -270))
    # prim_utils.set_prim_property("/World/BasicScene", "xformOp:orient", [0.7071, 0, 0, 0.7071])

    # context = omni.kit.commands.get_context()
    # context.execute("LoadUSD", usd_path="omni_drones/scenes/Templates/Basic/clean_cloudy_sky_and_floor.usd")
    # stage = get_current_stage()

    # stage = get_current_stage()

    # # define an empty Xform at /World/BasicScene
    # ref_xform = stage.DefinePrim("/World/BasicScene", "Xform")

    # # add a reference to the file—this will pull in exactly the same
    # # prim hierarchy and transforms that it was authored with
    # ref_xform.GetReferences().AddReference(
    #     assetPath="omni_drones/scenes/Templates/Basic/clean_cloudy_sky_and_floor.usd"
    # )
...
def create_ground_plane(
    ...
    **kwargs,
    ...
    geom_prim = GeometryPrim(collision_prim_path, disable_stablization=False, collision=collision)
    geom_prim.apply_physics_material(material)

    if not collision:
        # Path to your collision plane
        collision_plane_path = "/World/defaultGroundPlane/GroundPlane/CollisionPlane"

        # Get the collision plane prim
        collision_plane_prim = prim_utils.get_prim_at_path(collision_plane_path)

        # Apply CollisionAPI and explicitly disable collision
        collision_api = UsdPhysics.CollisionAPI.Apply(collision_plane_prim)
